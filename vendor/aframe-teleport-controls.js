...
/***/ (function(module, exports) {
	...
	var RayCurve = function (numPoints, width) {
	  ...
	  this.vertices = new Float32Array(numPoints * 3 * 2 * 2);
	  this.width = width;
	...
	RayCurve.prototype = {
	  ...
	  setPoint: (function () {
	    ...
	    return function (i, point) {
	      ...
	      function place_Vector3( vertex_index, vec ) {
	        this.vertices[ vertex_index*3 + 0 ] = vec.x;
	        this.vertices[ vertex_index*3 + 1 ] = vec.y;
	        this.vertices[ vertex_index*3 + 2 ] = vec.z;
	      }
	      function copy_vertex( dst_vertex_index, src_vertex_index ) {
	        this.vertices[ dst_vertex_index*3 + 0 ] = this.vertices[ src_vertex_index*3 + 0 ];
	        this.vertices[ dst_vertex_index*3 + 1 ] = this.vertices[ src_vertex_index*3 + 1 ];
	        this.vertices[ dst_vertex_index*3 + 2 ] = this.vertices[ src_vertex_index*3 + 2 ];
	      }
	      
          if( i === 0 ) {
            // The first two points
            place_Vector3_at( 0, posA );
            place_Vector3_at( 1, posB );
          }
	      else if( i === 1 ) {
	        // Finish the first triangle.
	        place_Vector3( 2, posA );
	        
	        // The second triangle.
	        copy_vertex(   3, 1 );
	        place_Vector3( 4, posB );
	        place_Vector3( 5, posA );
	      }
	      else
	      {
	        var idx = (i-1)*6;
	        // The first triangle.
	        copy_vertex(   idx+0, idx-1 );
	        copy_vertex(   idx+1, idx-2 );
	        place_Vector3( idx+2, posA );
	        
	        // The second triangle.
	        copy_vertex(   idx+3, idx-2 );
	        place_Vector3( idx+4, posB );
	        place_Vector3( idx+5, posA );
	      }
